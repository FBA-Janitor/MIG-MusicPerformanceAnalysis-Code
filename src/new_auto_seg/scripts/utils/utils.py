...
def read_annotation_as_seg(seg_csv):
    """
...
def read_annotation_as_label(time_stamp, seg_csv):
    """
    ...
    """
    seg = read_annotation_as_seg(seg_csv)
...
def load_data(
    ...
    """
    data_csv = read_multiple_csv(csv_list)
    data_X, data_y = [], []
    for stu_id, audio_path, segment_path in tqdm(data_csv):
        ...
        anno = read_annotation_as_label(time_stamp, segment_path)
...
def normalize_data(X, norm_param_path=norm_param_path):
    """
    ...
    """
    if not os.path.exists('{}.npz'.format(norm_param_path)):
        X_min = X.min(axis=0)
        X_max = X.max(axis=0)
